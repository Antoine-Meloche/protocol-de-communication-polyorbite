...
impl ReedSolomon {
    ...
    fn find_errors(
        &self,
        error_locator: &Bytes<{ N_K + 1 }>,
        received_len: usize,
    ) -> Bytes<{ T + 1 }> {
        let mut errors: Bytes<{ T + 1 }> = Bytes::<{ T + 1 }>::new();
        ...
        // Validate number of errors found matches degree of error locator
        if errors.pointer != error_locator.pointer - 1 {
            errors.push(1u8);
            return errors;
        }
    ...
    fn correct_errors(
        ...
        received: &mut [u8; N],
        syndromes: &[GF256; N_K],
        error_positions: &Bytes<T>,
    ) {
        let mut error_evaluator: Bytes<{ N_K }> = Bytes::<{ N_K }>::new();
        error_evaluator.push(syndromes[0].0);

        for i in 1..N_K {
            let mut sum = syndromes[i];
            for j in 1..=i {
                if j < error_positions.pointer {
                    sum = sum - (GF256(error_evaluator.bytes[i - j]) * syndromes[j]);
                }
            }
        ...
        for i in 0..error_positions.pointer {
            let mut sum = GF256(0);
            for j in 0..error_positions.pointer {
                if j != i {
                    let mut prod = GF256(1);
                    for k in 0..error_positions.pointer {
                        if k != i && k != j {
                            prod = prod * GF256(1) - GF256(error_positions.bytes[k]);
                        }
                    }
                    sum = sum + prod;
                }
        ...
        // Apply corrections
        for i in 0..error_positions.pointer {
            let pos = error_positions.bytes[i] as usize;
            let x = GF256(Self::exp_table(pos as u8));
            let magnitude =
                (GF256(error_evaluator.bytes[i]) * x) / GF256(locator_derivative.bytes[i]);
    ...
    pub fn decode(&self, received: &mut [u8; N]) -> Option<[u8; K]> {
        ...
        let error_positions: Bytes<{ T + 1 }> = self.find_errors(&error_locator, N);
        let mut error_positions_f: Bytes<T> = Bytes::<T>::new();

        if error_positions.pointer == T + 1 {
            return None;
        } else {
            error_positions_f.extend(&error_positions.bytes[0..T]);
        }

        self.correct_errors(received, &syndromes, &error_positions_f);
